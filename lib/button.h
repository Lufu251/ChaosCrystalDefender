...
struct button
{
    ...
    button(){}
    button(int xPosition, int yPosition, int xSize, int ySize, std::string text, int textSize):position(xPosition, yPosition), size(xSize,ySize), t(text), textS(textSize){}
};
    ...
    void drawButtons(sf::RenderWindow &pWindow){
        for (int i = 0; i < buttons.size(); i++)
        {
            ...
            sf::Text text;
                ...
                text.setCharacterSize(buttons[i].textS);
