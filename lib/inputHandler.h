...
class inputHandler
{
    ...
    void updateMouseWeel(int ticks){
        mouseWeel = ticks;
    }

    void resetKey(std::string name){
        mButtons[name].pressedOnce = false;
    }
    ...
    Eigen::Vector2i getMousePosition(){
        return Eigen::Vector2i(mousePosition.x(), mousePosition.y());
    }
    Eigen::Vector2i getMousePositionInRelationToView(sf::RenderWindow &rWindow){
        sf::Vector2f worldPos = rWindow.mapPixelToCoords(sf::Vector2i(mousePosition.x(), mousePosition.y()));
        return Eigen::Vector2i(worldPos.x, worldPos.y);
    }
    
    int getMouseWeelState(){
        return mouseWeel;
    }
